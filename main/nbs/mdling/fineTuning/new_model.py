...
#         X = tf.reshape(X, [-1, hparams.n_ctx, 2])
        ...
        # pool_idx = the index of the end of the sequence
#         pool_idx = tf.cast(tf.argmax(tf.cast(tf.equal(X[:, :, 0], hparams.n_vocab), tf.float32), 1), tf.int32)
#         clf_h = tf.gather(clf_h, tf.range(shape_list(X)[0], dtype=tf.int32)*hparams.n_ctx+pool_idx)
...
#             shape[1] = 1 # THis is for dropping out the different stories independently
            clf_h = tf.nn.dropout(clf_h, 1 - clf_pdrop)#, shape)
#         clf_h = tf.reshape(clf_h, [-1, hparams.n_embd])
        clf_logits = clf(clf_h, 1, train=train)[batch * 2]

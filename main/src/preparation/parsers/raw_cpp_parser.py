...
def func_name_extract(file_path):
	...
	for i in range(index):
		...
		if curr.type == 'function_definition':
			if curr.children[0].type == 'class_identifier':
				curr = curr.children[0]
				temp = 0
				while temp != len(curr.children) and curr.children[temp].type != 'field_declaration_list':
					temp += 1
				if temp == len(curr.children):
					continue
				curr = curr.children[temp]
				for x in range(len(curr.children)):
					if curr.type == 'function_definition':
						startline = curr.start_point
						endline = curr.end_point
						declare = 0
						while declare != len(curr.children) and curr.children[declare].type != 'function_declarator':
							print(curr.children[declare].type)
							print(curr.children[declare])
							declare += 1
						if declare == len(curr.children):
							continue
						identify = 0
						while curr.children[declare].children[identify].type != 'identifier' and curr.children[declare].children[identify].type != 'field_identifier':
							identify += 1
						startname = curr.children[declare].children[identify].start_point
						endname = curr.children[declare].children[identify].end_point
				...
				if declare == len(curr.children):
					continue
				identify = 0
				while identify != len(curr.children[declare].children) and curr.children[declare].children[identify].type != 'field_identifier' and curr.children[declare].children[identify].type != 'identifier':
					identify += 1
				if identify == len(curr.children[declare].children):
					continue
				startname = curr.children[declare].children[identify].start_point
				endname = curr.children[declare].children[identify].end_point
				with open(file_path, "r", encoding='UTF-8') as f:
